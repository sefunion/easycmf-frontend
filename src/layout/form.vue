<template>
  <div class="easy-content-block p-4">
    <a-page-header
      ...
      @back="pageBack"
    >
      <easy-form v-model="form" :columns="formConfig?.formColumns ?? []" ref="easyFormRef" @onSubmit="submitForm" />
    </a-page-header>
  </div>
</template>
<script setup>
...
const submitForm = async () => {
  const formData = easyFormRef.value.getFormData()
  if (await easyFormRef.value.validateForm()) {
    return false
  }
  ...
  if ( response.success ) {
    await easyFormRef.value.resetForm()
