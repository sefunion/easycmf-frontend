<template>
  <div class="w-full p-2 resource-container h-full lg:flex lg:justify-between rounded-sm">
    <a-modal v-model:visible="openNetworkModal" :ok-text="$t('sys.save')" :on-before-ok="saveNetworkImg" draggable>
      <template #title>{{ $t('easyResource.saveNetworkImage') }}</template>
      <a-input v-model="networkImg" class="mb-3" :placeholder="$t('easyResource.networkImageNotice')" allow-clear />
      <a-image :src="networkImg" width="100%" style="min-height: 150px;" />
    </a-modal>
    <div class="lg:w-1/5 w-full p-2 shadow">
      <easy-tree-slider
        v-model="sliderData"
        :search-placeholder="$t('easyResource.searchResource')"
    ...
    <div class="w-full lg:ml-3 mt-3 lg:mt-2 flex flex-col">
      ...
      <div class="lg:flex lg:justify-between">
        <div class="flex">
          <easy-upload v-model="uploadFile" multiple :show-list="false" type="chunk" :resource="false" />
          <a-button class="ml-3" @click="openNetworkModal = true">
            <icon-image /> {{ $t('easyResource.saveNetworkImage') }}
          </a-button>
        </div>
        <a-input
          v-model="filename"
          class="input-search lg:mt-0 mt-2"
          :placeholder="$t('easyResource.searchFileNotice')"
          @press-enter="searchFile"
        />
      </div>
      <a-spin :loading="resourceLoading" :tip="$t('easyResource.loadingText')" class="h-full">
      ...
      <div class="lg:flex lg:justify-between">
        <a-pagination
          ...
          @change="changePage"
        />
        <a-button type="primary" @click="selectComplete" class="mt-3 lg:mt-0">{{ $t('easyResource.ok') }}</a-button>
      </div>
    </div>
  </div>
</template>
...
<script setup>
  ...
  import EasyUpload from '@cps/easy-upload/index.vue'
  import uploadConfig from '@/config/upload'
  import EasyTreeSlider from '@cps/easy-treeSlider/index.vue'
  ...
  const saveNetworkImg = async(done) => {
    if (! networkImg.value) {
      Message.error(t('easyResource.networkImageNotice'))
      done(false)
      return
    }
