...
import EasyFormInput from './searchFormItem/form-input.vue'
import EasyFormPicker from './searchFormItem/form-picker.vue'
import EasyFormSelect from './searchFormItem/form-select.vue'
import EasyFormCascader from './searchFormItem/form-cascader.vue'
import EasyFormTreeSelect from './searchFormItem/form-tree-select.vue'
...
const gridClass = ref(['easy-search-grid', 'w-full', 'grid', 'lg:grid-cols-' + options.searchColNumber ?? 4])
...
const componentList = ref({
  'EasyFormSelect': markRaw(EasyFormSelect),
  'EasyFormPicker': markRaw(EasyFormPicker),
  'EasyFormCascader': markRaw(EasyFormCascader),
  'EasyFormTreeSelect': markRaw(EasyFormTreeSelect),
  'EasyFormInput': markRaw(EasyFormInput),
}) 

const getComponentName = (formType) => {
  if (['select', 'radio', 'checkbox', 'transfer'].includes(formType)) {
    return componentList.value['EasyFormSelect']
  } else if (['date', 'month', 'year', 'week', 'quarter', 'range', 'time'].includes(formType)) {
    return componentList.value['EasyFormPicker']
  } else if (formType === 'cascader') {
    return componentList.value['EasyFormCascader']
  } else if (formType === 'tree-select') {
    return componentList.value['EasyFormTreeSelect']
  } else {
    return componentList.value['EasyFormInput']
  }
}
