<template>
  <ul
    class="easy-crud-contextmenu shadow-lg"
...
watch(
    ...
    value => {
      ...
      const crudContextMenuhandler = (e) => {
        const dom = document.querySelector('.easy-crud-contextmenu')
...
const openContextMenu = async (ev, record) => {
  ...
  await nextTick(() => {
    const domHeight = document.querySelector('.easy-crud-contextmenu').offsetHeight
