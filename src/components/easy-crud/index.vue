  ...
  <a-layout-content class="flex flex-col lg:h-full relative w-full">
    <div class="_crud-header flex flex-col mb-2" ref="crudHeaderRef">
      <a-tabs
          ...
          :hide-content="true"
          @change="tabChange"
          @tab-click="easyEvent.customeEvent(options.tabs, $event, 'onClick')"
          class="easy-tabs mb-5"
      >
    ...
    <easy-setting ref="crudSettingRef" />

    <easy-form ref="crudFormRef" @success="requestSuccess">
      <template v-for="slot in Object.keys($slots)" #[slot]="component">
        <slot :name="slot" v-bind="component" />
      </template>
    </easy-form>

    <easy-import ref="crudImportRef" />

    <easy-context-menu ref="crudContextMenuRef" @execCommand="execContextMenuCommand" />
...
import { loadDict } from '@cps/easy-form/js/networkRequest.js'
import ColumnService from './js/columnService'

import EasySearch from './components/search.vue'
import EasyForm from './components/form.vue'
import EasySetting from './components/setting.vue'
import EasyImport from './components/import.vue'
import EasyColumn from './components/column.vue'
import EasyContextMenu from './components/contextMenu.vue'
...
import { easyEvent } from '@cps/easy-form/js/formItemMixin.js'
...
const requestHandle = async () => {
  ...
  if (isFunction(currentApi.value)) {
    ...
    console.error(`easy-crud error：crud.api not is Function.`)
  }
...
const tabChange = async (value) => {
  ...
  await easyEvent.customeEvent(options.value.tabs, value, 'onChange')
  await refresh()
}
...
const execContextMenuCommand = async (args) => {
  ...
  switch(item.operation) {
    ...
    default:
      await easyEvent.customeEvent(item, args, 'onCommand')
      break;
  }
}
