  ...
  <div class="w-full">
    ...
    <a-modal v-model:visible="visible" width="800px" draggable :footer="false">
      ...
      <a-tabs class="tabs">
        ...
        <a-tab-pane key="easy" title="EasyCMF">
          <ul class="easy">
            <li
              v-for="icon in easycmfIcons"
              :key="icon"
              @click="selectIcon(icon, 'easy')"
            >
              <component :is="icon" />
            </li>
          </ul>
...
<script setup>
  ...
  const modules = import.meta.globEager('../../assets/easy-icons/*.vue')
