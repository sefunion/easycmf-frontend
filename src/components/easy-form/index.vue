  ...
  <div class="w-full">
    <a-spin :loading="formLoading" :tip="options.loadingText" class="w-full">
      <div
        ...
        :class="['easy-form-title', options.formTitleClass]"
      ...
      <a-form
        ref="easyFormRef"
        :model="form"
        :class="['easy-form', options?.customClass]"
...
easyEvent.handleCommonEvent(options.value, 'onCreated')

onMounted(async () => {
  easyEvent.handleCommonEvent(options.value, 'onMounted')
  options.value.init && await init()
  easyEvent.handleCommonEvent(options.value, 'onInit')
})

const done = (status) => formLoading.value = status
const validateForm = async () => {
  const valid = await easyFormRef.value.validate()
  if ( valid ) {
    let message = ''
...
const resetForm = async() => await easyFormRef.value.resetFields()
const clearValidate = async() => await easyFormRef.value.clearValidate()

const formSubmit = async () =>  (await validateForm() && !formLoading.value ) || emit('onSubmit', form.value, done)

const getFormRef = () => easyFormRef.value
