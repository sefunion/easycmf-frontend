<template>
  <a-modal width="1100px" v-model:visible="visible" :on-before-ok="loadTable" :align-center="false" top="50px" unmount-on-close>
    ...
    <!-- CRUD 组件 -->
    <easy-crud :options="crud" :columns="columns" ref="crudRef" @selection-change="handlerSelection">
...
<script setup>
  ...
  const switchSource = () => {
    ...
    crud.api = sourceName.value === 'EasyCMF' ? dataMaintain.getPageList : dataSource.getDataSourceTablePageList
    crudRef.value.requestParams['id'] = sourceName.value === 'EasyCMF' ? undefined : sourceName.value
