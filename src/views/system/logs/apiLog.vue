...
<template>
  <div class="easy-content-block lg:flex justify-between p-4">
    <!-- CRUD 组件 -->
    <easy-crud :options="crud" :columns="columns" @row-click="openDetails" ref="crudRef"></easy-crud>
    ...
    <a-drawer v-model:visible="visible" width="600px" :footer="false">
      ...
      >
        <a-collapse-item header="请求数据" key="request">
          <easy-code-editor v-model="currentRow.request_data" v-if="visible" :height="150" readonly />
        </a-collapse-item>
        <a-collapse-item header="响应数据" key="response">
          <easy-code-editor v-model="currentRow.response_data" v-if="visible" :height="150" readonly />
        </a-collapse-item>
      </a-collapse>
    </a-drawer>
  </div>
...
<script setup>
  ...
  import EasyCodeEditor from '@cps/easy-codeEditor/index.vue'
