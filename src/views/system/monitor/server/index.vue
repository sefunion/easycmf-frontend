...
<template>
  <a-layout-content>
    <div class="easy-content-block lg:flex p-4" v-if="cpu">
      ...
      <div class="flex justify-between w-full redis-info mt-3" v-else>
        ...
        <div class="echarts hidden lg:block">
          <easy-chart :options="cpuOptions" width="350px" height="350px" />
        </div>
      </div>
    </div>

    <div class="easy-content-block lg:flex p-4 mt-3" v-if="memory">
      <a-skeleton animation v-if="loading" class="w-full">
        <a-space direction="vertical" class="w-full" size="large">
          <a-skeleton-line :rows="5" />
        </a-space>
      ...
      <div class="flex justify-between w-full redis-info mt-3" v-else>
        ...
        <div class="echarts hidden lg:block">
          <easy-chart :options="memoryOptions" width="350px" height="350px" />
        </div>
      </div>
    </div>

    <div class="easy-content-block lg:flex p-4 mt-3" v-if="phpenv">
      <a-skeleton animation v-if="loading" class="w-full">
        <a-space direction="vertical" class="w-full" size="large">
          <a-skeleton-line :rows="5" />
        </a-space>
      ...
      <div class="flex justify-between w-full redis-info mt-3" v-else>
        <a-descriptions :column="2" size="large" bordered title="PHP及环境信息" class="w-full">
          ...
          <a-descriptions-item label="EasyCMF版本">{{ phpenv.easycmf_version }}</a-descriptions-item>
